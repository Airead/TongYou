...
public final class AppControlClient {
    ...
    /// When `noNotifyIfVisible` is true, skips the macOS notification if the
    /// target pane's tab is currently visible.
    public func notify(ref: String, title: String, body: String?, noNotifyIfVisible: Bool = false) throws {
        var params: [String: Any] = ["ref": ref, "title": title]
        if let body { params["body"] = body }
        if noNotifyIfVisible { params["noNotifyIfVisible"] = true }
