...
struct ConfigParserTests {
    ...
    @Test func maxIncludeDepth() throws {
        let dir = NSTemporaryDirectory() + "tongyou-test-\(UUID().uuidString)/"
        try FileManager.default.createDirectory(atPath: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(atPath: dir) }

        // Create a chain of 12 includes (exceeds max depth of 10)
        for i in 0..<12 {
            let url = URL(fileURLWithPath: dir + "file\(i).config")
            let next = URL(fileURLWithPath: dir + "file\(i + 1).config")
            if i < 11 {
                try "config-file = \(next.path)".write(to: url, atomically: true, encoding: .utf8)
            } else {
                try "font-size = 14".write(to: url, atomically: true, encoding: .utf8)
            }
        }

        let root = URL(fileURLWithPath: dir + "file0.config")
        #expect(throws: ConfigError.self) {
            try parser.parse(contentsOf: root)
        }
    }
