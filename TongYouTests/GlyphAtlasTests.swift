...
struct GlyphAtlasTests {
    ...
    @Test func getReturnsNilOnCacheMiss() {
        ...
        let font = fontSystem.ctFont
        guard let glyph = fontSystem.glyphForCharacter("Z", in: font) else {
            Issue.record("Font does not support character Z")
            return
        }
        let info = atlas.get(glyph: glyph, font: font)
        #expect(info == nil)
        #expect(atlas.isDirty == false)
    }

    @Test func getReturnsInfoAfterSyncRasterize() async throws {
        guard let (atlas, fontSystem) = makeAtlas() else {
            Issue.record("Metal device not available")
            return
        }
        let font = fontSystem.ctFont
        guard let glyph = fontSystem.glyphForCharacter("Y", in: font) else {
            Issue.record("Font does not support character Y")
            return
        }
        _ = atlas.getOrRasterize(glyph: glyph, font: font, fontSystem: fontSystem)
        atlas.clearDirty()
    ...
    @Test func asyncRasterizationFillsCacheAndIncrementsModified() async throws {
        ...
        let font = fontSystem.ctFont
        guard let glyph = fontSystem.glyphForCharacter("X", in: font) else {
            Issue.record("Font does not support character X")
            return
        }
        ...
        await withCheckedContinuation { continuation in
            atlas.enqueueRasterization(glyph: glyph, font: font, fontSystem: fontSystem) {
                continuation.resume()
            }
        }
    ...
    @Test func asyncRasterizationDeduplicatesPendingWork() async throws {
        ...
        let font = fontSystem.ctFont
        guard let glyph = fontSystem.glyphForCharacter("W", in: font) else {
            Issue.record("Font does not support character W")
            return
        }
        ...
        await withCheckedContinuation { continuation in
            ...
            // Second enqueue for the same glyph should be deduplicated
            atlas.enqueueRasterization(glyph: glyph, font: font, fontSystem: fontSystem) {
                completionCount += 1
            }
        }

        // Both completions should fire, but only one rasterization task ran
        #expect(completionCount == 2)
        #expect(atlas.get(glyph: glyph, font: font) != nil)
    }

    @Test func asyncRasterizationDiscardsStaleWorkAfterReset() async throws {
        guard let (atlas, fontSystem) = makeAtlas() else {
            Issue.record("Metal device not available")
            return
        }
        let font = fontSystem.ctFont
        guard let glyph = fontSystem.glyphForCharacter("V", in: font) else {
            Issue.record("Font does not support character V")
            return
        }

        let initialGeneration = atlas.generation

        await withCheckedContinuation { continuation in
            atlas.enqueueRasterization(glyph: glyph, font: font, fontSystem: fontSystem) {
                continuation.resume()
            }
            // Reset immediately while rasterization is in flight
            atlas.reset()
        }

        #expect(atlas.generation > initialGeneration)
        // Stale work should not have added the glyph back
        #expect(atlas.get(glyph: glyph, font: font) == nil)
    }
}
